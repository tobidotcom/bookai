...
def generate_outline(prompt):
    ...
    response = client.chat.completions.create(
        model="gpt-4o", messages=messages, max_tokens=1024, n=1, stop=None, temperature=0.7
    )
...
def main():
    ...
    if st.button("Generate Outline"):
        if prompt.strip():
            ...
            st.write("Outline:")
            st.write(outline)
            # Enable the "Generate Pre-Summary" button
            pre_summary_button = st.button("Generate Pre-Summary", disabled=False)
        else:
            st.warning("Please enter a book prompt first.")
            pre_summary_button = st.button("Generate Pre-Summary", disabled=True)

    # Add this line to disable the "Generate Pre-Summary" button initially
    pre_summary_button = st.button("Generate Pre-Summary", disabled=True)

    if pre_summary_button:
        if not prompt.strip():
            st.warning("Please enter a book prompt first.")
            ...
            # Enable the "Generate Chapters" button
            st.button("Generate Chapters", disabled=False)
